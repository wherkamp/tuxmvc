...
import me.kingtux.tuxmvc.core.view.TemplateGrabbers;
...
public class WebsiteBuilder {
    ...
    public static Website buildFromProperties(Properties p) {
        ...
        TemplateGrabber tg = TemplateGrabbers.valueOf(p.getProperty("template.grabber")).build(p.getProperty("template.extension"), p.getProperty("template.path"));
        EmailManager em = SimpleEmailManager.buildEmailManager(p.getProperty("email.host"), p.getProperty("email.port"), TransportStrategy.valueOf(p.getProperty("email.ts")), p.getProperty("email.from"), p.getProperty("email.password"), p.getProperty("email.from.name"));
        TuxJSQL.setup(p);
        ...
        return new SimpleWebsite(new WebsiteRules(factory != null ? "https" : "http", host), port, file, tg, em, dbManager, factory, sslPort);
